...
Render into slides folder
    manim-slides render simulation.py Simulation
...
class Simulation(Slide):
    ...
    def construct(self):
        ...
        for x in range(80, 760, 80):
            grid.add(Line(s(x, 0), s(x, 360), color=GRID_COLOR,
                          stroke_width=0.4, stroke_opacity=0.15))
        for y in range(60, 360, 60):
            grid.add(Line(s(0, y), s(760, y), color=GRID_COLOR,
                          stroke_width=0.4, stroke_opacity=0.15))
        self.add(grid)
