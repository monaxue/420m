...
::: notes
This week we will simulate sex differences in height, use group_by, change colors in the plot, use facet_wrap, and create a summary table.

Talk about how we can't draw conclusions about differences from descriptive plots alone.
:::

As always:
1. Create your lab4 folder
1. Create your R studio project
1. Create your lab4.Rmd file
1. Add the title, author, and date to the Rmd file.
1. Load `tidyverse` and `ggplot2` using the `library()` function.

Additionally:
1. Copy the heights data that you simulated from last week into the lab4 folder.
1. Load the data into lab4.Rmd using read_csv()
...
## 
Last week, we ended by simulating female heights. Now, let's simulate male heights.
::: card-list
    - 
:::
...
- You can chain pipes together, like so
    ...
    - `df <- df %>% mutate(sex="f")) %>% mutate(mean=mean(height))`
...
- Let's take a look in R
    ...
    - What happens to the `data` variable? 
