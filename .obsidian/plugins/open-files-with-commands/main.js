...
var FileSuggest = class extends TextInputSuggest {
  ...
  getSuggestions(input_str) {
    ...
    this.plugin.app.vault.getFiles().forEach((file) => {
      if (file instanceof import_obsidian2.TFile && file.path.toLowerCase().contains(lower_input_str)) {
        files.push(file);
      }
    });
    return files;
...
var FileCommand = class {
  constructor(name, file, plugin, id) {
    ...
    return this.command = plugin.addCommand({
      ...
      checkCallback(checking) {
        ...
        (async () => {
          ...
          if (!(file instanceof import_obsidian3.TFile)) {
            new import_obsidian3.Notice(`File "${filePath}" not found`);
            return false;
          }
          const files = [];
          let leaf = null;
  ...
  updateCommand(newFileCommand) {
    var _a;
    if (newFileCommand.id != this.id)
      return false;
    ...
    if (fileWithSamePath && newFileCommand.filePath != "") {
      new import_obsidian3.Notice(`A command with the same file path already exists: "${fileWithSamePath.name}"`);
      newFileCommand.filePath = ((_a = commands.find((e) => e.id == newFileCommand.id)) == null ? void 0 : _a.filePath) || "";
      return false;
    }
    if (pathIsDifferent) {
...
async function replaceArgs(filePath, plugin) {
...
async function replaceVariables(filePath, plugin, args) {
  for (const arg of args) {
    ...
    if (customVariable.type === "javascript") {
      ...
      const args2 = customVariable.value.match(/\{\{([^}]+)\}\}/g);
      let customVariableCopy = Object.assign({}, customVariable);
      if (args2) {
        const result = await replaceVariables(customVariable.value, plugin, args2);
        if (result !== void 0) {
...
function getLeaf(plugin, leaf, fileCommand, files = [], file) {
  switch (fileCommand == null ? void 0 : fileCommand.openFileIn) {
    case "activeTab":
    ...
    case "newTabSplit":
      ...
      break;
    case "rightLeaf":
      plugin.app.workspace.getLeavesOfType("markdown").forEach((leaf2) => {
        if (leaf2.getRoot() === plugin.app.workspace.rightSplit && leaf2.view instanceof import_obsidian3.FileView && leaf2.view.file.path == file.path) {
          if (leaf2.view instanceof import_obsidian3.FileView && leaf2.view.file.path == file.path) {
            plugin.app.workspace.revealLeaf(leaf2);
            return files.push(leaf2.view.file);
          }
        }
      });
      if (files.length > 0)
    ...
    case "leftLeaf":
      plugin.app.workspace.getLeavesOfType("markdown").forEach((leaf2) => {
        if (leaf2.getRoot() === plugin.app.workspace.leftSplit && leaf2.view instanceof import_obsidian3.FileView && leaf2.view.file.path == file.path) {
          plugin.app.workspace.revealLeaf(leaf2);
          return files.push(leaf2.view.file);
        }
      });
...
// src/manageVariables.ts
var import_obsidian4 = require("obsidian");
var ManageVariablesModal = class extends import_obsidian4.Modal {
  ...
  display() {
    ...
    new import_obsidian4.Setting(contentEl).setName("Manage Variables").setDesc(desc).addButton((button) => {
...
function addVariableListItem(settings, containerEl, variable, modal) {
  const plugin = settings.plugin;
  const setting = new import_obsidian4.Setting(containerEl);
  setting.setClass("variable-item");
  setting.addText((text) => text.setPlaceholder("Variable Name").setValue(variable.name).onChange(
    async (value) => {
      variable.name = value;
      await plugin.saveSettings();
    }
  ));
  setting.addTextArea((text) => text.setPlaceholder("Variable Value").setValue(variable.value).onChange(
    async (value) => {
      variable.value = value;
      await plugin.saveSettings();
    }
  ));
  setting.addDropdown((dropdown) => dropdown.addOption("string", "String").addOption("javascript", "JavaScript").setValue(variable.type).onChange(
    async (value) => {
      variable.type = value;
      await plugin.saveSettings();
    }
  ...
  setting.addButton((button) => {
    button.setIcon("trash").setTooltip("Delete Variable").onClick(() => {
...
function createVariable(plugin) {
  const variable = {
    name: "",
    value: "",
    type: "string"
...
function deleteVariable(plugin, variable) {
...
var SettingsTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    if (import_obsidian5.Platform.isDesktopApp) {
      new import_obsidian5.Setting(containerEl).setName("Choose where you want to open the files").addDropdown((cb) => {
        cb.addOptions(
          {
        ...
        cb.onChange(async (val) => {
          this.plugin.settings.openFileIn = val;
          await this.plugin.saveSettings();
        });
      });
    }
    new import_obsidian5.Setting(containerEl).setName("Delete command when file is deleted").setDesc("If you delete a file, the command for that file will be deleted as well").addToggle((cb) => {
    ...
    new import_obsidian5.Setting(containerEl).setName("Update command when file is renamed").setDesc("If you rename a file, the command for that file will be updated as well").addToggle((cb) => {
    ...
    new import_obsidian5.Setting(containerEl).setName("Custom Variables").addButton((button) => {
      button.setButtonText("Manage Variables").onClick(() => {
        new ManageVariablesModal(this.app, this.plugin, this).open();
      });
    });
    new import_obsidian5.Setting(containerEl).setHeading().setClass("ofwc-commands-heading").setName("Manage commands");
    new import_obsidian5.Setting(containerEl).setName("Create a new command").addButton((cb) => {
      cb.setIcon("plus");
  ...
  async addCommandListOption(containerEl, fileCommand) {
    ...
    setting.addButton((cb) => {
      cb.onClick(() => {
        ...
        } else if (((_a = fileCommand.filePath) == null ? void 0 : _a.trim()) !== "") {
    ...
    setting.addSearch((s) => {
      ...
      s.onChange(async () => {
        ...
        if (this.plugin.commands.findIndex((e) => (e == null ? void 0 : e.id) == fileCommand.id) == -1) {
          if (this.plugin.settings.commands.some((e) => (e == null ? void 0 : e.id) == fileCommand.id)) {
            ...
            if (newFileCommand) {
              ...
              new import_obsidian5.Notice("File already has a command");
              return this.display();
            }
          }
        } else {
          ...
          if (!fileCommand.updateCommand(fileCommand)) {
            fileCommand.filePath = ((_a = this.plugin.settings.commands.find((e) => (e == null ? void 0 : e.id) == fileCommand.id)) == null ? void 0 : _a.filePath) || fileCommand.filePath;
            this.plugin.commands[this.plugin.commands.findIndex((e) => (e == null ? void 0 : e.id) == fileCommand.id)] = fileCommand;
            this.display();
          }
        }
      });
  ...
  createFileCommand(name, file, plugin, fileCommand) {
    if (!fileCommand) {
  ...
  saveFileCommand(fileCommand, settingsOnly) {
    if (!settingsOnly) {
      this.plugin.commands.push(fileCommand);
    }
    this.plugin.settings.commands.push({ id: fileCommand.id, name: fileCommand.name, filePath: fileCommand.filePath, openFileIn: fileCommand.openFileIn, command: fileCommand.command });
    this.plugin.saveSettings();
  }
  async addCommand(name, filePath, plugin, id, notice) {
    if (this.plugin.settings.commands.some((e) => (e == null ? void 0 : e.filePath) == filePath)) {
      return false;
    }
...
// src/main.ts
var OpenFilesPlugin = class extends import_obsidian6.Plugin {
  constructor() {
    super(...arguments);
    this.commands = [];
  }
  async onload() {
    ...
    this.registerEvent(this.app.workspace.on("file-menu", (menu, file) => {
      ...
      if (!fileCommand) {
        menu.addItem(
          (item) => item.setTitle("Create a command for this file").setIcon("command").onClick(() => {
        ...
        menu.addItem(
          (item) => item.setTitle("Delete the command for this file").setIcon("trash").onClick(() => {
    ...
    this.registerEvent(this.app.vault.on("rename", (file, oldPath) => {
      if (!(file instanceof import_obsidian6.TFile))
        return;
      if (!this.settings.updateCommandsOnRename)
        return;
    ...
    this.registerEvent(this.app.vault.on("delete", (file) => {
      if (!this.settings.deleteCommandWhenFileIsDeleted)
        return;
      const fileCommand = this.commands.find((c) => c.filePath == file.path);
      if (fileCommand) {
        this.settingsTab.deleteCommand(fileCommand.id);
