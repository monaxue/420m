...
:::{.column}
:::{.card-list}
- We wil by trying Slido for attendence.

- Scan the code below and create a profile.
...
- Basic r syntax: comments, assign, varibles, strings, lists, operators, print, indeces
- R coding challenge
- Installing packages
- Reading data
...
**This is how you print values so that they are visable**
```r
print(x)
```
...
## 
:::{.card-list}
- Your friend is trying to send you his secret password. He gave you the word "Random", then provided a list instructions on how to decode it.
- R Cheatsheet
    - ![](assets/r-cheatsheet.png)
...
- Each number in vector `e` represents the position of a letter in `word3`. This position is called the index. A new word is formed when you reorder the letters based on the order specifed in `e`. Print out the new word. Challenge mode: decipher the word in one line of code using `[]`
...
# Each number in vector e represents the position of a letter in `word3`. This position is called the index. A new word is formed when you reorder the letters based on the order specifed in `e`. Print out the new word. Challenge mode: decypher the word in one line of code using []
...
# Install tidyverse (important package for data analysis)
